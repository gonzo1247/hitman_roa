...
switch ($mode)
{
		...
		switch ($give)
		{
			...
			$sql = 'SELECT COUNT(user_id) AS total_match_count
				FROM ' . THANKS_TABLE . '  
				WHERE (' . $db->sql_in_set('forum_id', $ex_fid_ary, true) . ' OR forum_id = 0) AND user_id = '.$author_id; 
			$where = 'user_id';
			break;
			...
			$sql = 'SELECT COUNT(DISTINCT post_id) as total_match_count
				FROM ' . THANKS_TABLE . '  
				WHERE (' . $db->sql_in_set('forum_id', $ex_fid_ary, true) . ' OR forum_id = 0) AND poster_id = '.$author_id; 
			$where = 'poster_id';
			break;		
		}
		...
		else
		{
			...
			$sql_array = array(
				...
				'WHERE'		=> '('. $db->sql_in_set('t.forum_id', $ex_fid_ary, true) . ' OR t.forum_id = 0) AND t.' . $where . "= $author_id"
			);
		...
		$sql = 'SELECT user_id, COUNT(user_id) AS tally
			FROM ' . THANKS_TABLE . '  
			WHERE ' . $db->sql_in_set('forum_id', $ex_fid_ary, true) . ' OR forum_id = 0
			GROUP BY user_id';
		$result = $db->sql_query($sql);
		while ($row = $db->sql_fetchrow($result))
		{
