...
$end_row_rating = isset($config['thanks_number_row_reput']) ? $config['thanks_number_row_reput'] : false;
...
//post rating
	if (!$full_forum_rating && !$full_topic_rating && $config['thanks_post_reput_view'])
	{
		...
		$sql_p_array['WHERE'] = $db->sql_in_set('t.forum_id', $ex_fid_ary, true);		
		...
		else
		{
			...
			do
			{
				...
				$template->assign_block_vars('toppostrow', array(
					...
					'S_THANKS_POST_REPUT_VIEW' 	=> isset($config['thanks_post_reput_view']) ? $config['thanks_post_reput_view'] : false,
...
//topic rating
	if (!$full_forum_rating && !$full_post_rating && $config['thanks_topic_reput_view'])
	{
		...
		$sql_t_array['WHERE'] = $db->sql_in_set('f.forum_id', $ex_fid_ary, true);	
		...
		else
		{
			...
			do
			{
				...
				$template->assign_block_vars('toptopicrow', array(
					'TOPIC_FOLDER_IMG_SRC'		=> $row['forum_id'] ? $user->img('topic_read', 'NO_NEW_POSTS', false, '', 'src') : $user->img('announce_read', 'NO_NEW_POSTS', false, '', 'src'),
...
//forum rating
	if (!$full_topic_rating && !$full_post_rating && $config['thanks_forum_reput_view'])
	{
		...
		$sql_f_array['WHERE'] = $db->sql_in_set('t.forum_id', $ex_fid_ary, true);	
